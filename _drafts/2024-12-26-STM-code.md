...
(The statement `refVals <- refVals.Next` is doing the heavy lifting here; it rotate the oldet entry to be the root and changes its value. Draw pictures.)
...
### Running a transaction
 ...
 The Clojure-level macros `dosync` and `sync` both expand in call to `LockingTransaction.runInTransaction`.
...
Also, `Ref`'s implemention of `IDeref.deref` calls `doGet`.  Let's start with the easiest one: `doGet`.
...
Okay.  That's the easy one.  `doCommute` is not harder.
...
That wasn't so bad.
  ...
  The `lock` method is hiding the nastiness.
