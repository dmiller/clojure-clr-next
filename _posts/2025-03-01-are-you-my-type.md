...
To these two methods, I added two others: a map of dynamically created types and a parser for type names.  The map is a cache of types that have been generated by the REPL or by AOT-compilation. The types searched for typically are deftypes and protocols.  One sees a search for `"clojure.core.VecSeq`", a type defined in `gvec.clj`, succeed by map lookup.

The parser is the last ditch attempt to deal with a given string.  Instantiated generic types and array types are examples of strings that need to be parsed.  (Though as mentioned, `Type.GetType()` will instantiate some generic types for you.) There are not many of these looked up during the core Clojure load.  We might run into it more in user code.  Given `"System.Collections.Generic.List`1[System.String]"`, the parser will actually instantiate the generic type on the fly.  

`RT.classForName` in ClojureCLR attempts the following:

1. Look up in the map of generated types 
2. Direct lookup via `Type.GetType()`
...
For failures, `Assembly.GetType()` is slower --we have to iterate through all assemblies.  I'm guessing `System.String`'s assembly is about one third of the way down the list.

 The relative performance of `Assembly.GetType()` worsens as we add assemblies -- and here we have the tie-in to the popcorn-popping proliferation of assemblies mentioned at the start.   In the test project, I added 100 dynamic assemblies with 5 classes each.  The times for the same 100,000 lookups were:
