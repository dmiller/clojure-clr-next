...
type BigIntegerExtensions private () =
    ...
    static let BitsPerDigit = 32; // uint implementation
    ...
    static let RadixDigitsPerDigit = 
        [| 0; 0;
           31; 20; 15; 13; 12;
           11; 10; 10; 9; 9; 
            8; 8; 8; 8; 7;
    ...
    static member Parse(s: string, radix: int) =
    ...
    static member TryParse(s: string, radix: int) : BigInteger option =
        ...
        if radix < MinRadix || radix > MaxRadix then
    ...
    static member TryParseAux(s: string, index: int, sign: int, radix: int) : BigInteger option = 
        let len = s.Length
        ...
        let numBits = ((numDigits * BitsPerRadixDigit[radix]) >>> 10) + 1
        let numUints = (numBits + BitsPerDigit - 1) / BitsPerDigit

        let groupSize = RadixDigitsPerDigit[radix]
        ...
        | Some firstDigit ->
            ...
            let mult = BigInteger(SuperRadix[radix])
    ...
    static member TryParseUInt(value: string, startIndex: int, len: int, radix: int) : uint option =
    ...
    static member TryComputeDigitVal(c : char, radix: int) : uint option =
        let v = 
            if '0' <= c && c <= '9' then
                uint c - uint '0'
            elif 'a' <= c && c <= 'z' then
