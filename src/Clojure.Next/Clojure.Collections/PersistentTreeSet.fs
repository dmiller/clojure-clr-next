...
type PersistentTreeSet private (_meta: IPersistentMap, _impl: IPersistentMap) = 
    ...
    interface IObj with
        override this.withMeta (meta: IPersistentMap): IObj = 
            if Object.ReferenceEquals(meta,_meta) then this
            else PersistentTreeSet(meta,_impl)
    ...
    interface IPersistentCollection with
        ...
        override this.cons (o: obj): IPersistentCollection = 
            if (this :> IPersistentSet).contains(o) then this else PersistentTreeSet(_meta,_impl.assoc(o,o))
    ...
    interface Reversible with
        member _.rseq (): ISeq = KeySeq.create((_impl :?> Reversible).rseq())
