...
/// Interface for sequences that are backed by an array.
[<AllowNullLiteral>]
type IArraySeq =
    inherit IObj
    inherit ISeq
...
/// A sequenced backed by an array of elements of a given type.
[<AllowNullLiteral>]
type TypedArraySeq<'T when 'T: equality>(_meta: IPersistentMap, _array: 'T array, _index: int) =
    ...
    default _.NextOne() =
        new TypedArraySeq<'T>(_meta, _array, _index + 1)

    default this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new TypedArraySeq<'T>(newMeta, _array, _index)
    ...
    interface ISeq with
        ...
        override this.next() =
            if _index + 1 < _array.Length then this.NextOne() else null
...
/// Base class for sequences backed by an array of primitive numeric element type.
...
/// Sequence over an array of bytes.
[<AllowNullLiteral>]
type ArraySeq_byte(_meta: IPersistentMap, _array: byte array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_byte(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_byte(newMeta, _array, _index)

/// Sequence over an array of signed bytes.
[<AllowNullLiteral>]
type ArraySeq_sbyte(_meta: IPersistentMap, _array: sbyte array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_sbyte(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_sbyte(newMeta, _array, _index)

/// Sequence over an array of shorts.
[<AllowNullLiteral>]
type ArraySeq_short(_meta: IPersistentMap, _array: int16 array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_short(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_short(newMeta, _array, _index)

/// Sequence over an array of ushorts.
[<AllowNullLiteral>]
type ArraySeq_ushort(_meta: IPersistentMap, _array: uint16 array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_ushort(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_ushort(newMeta, _array, _index)

/// Sequence over an array of ints.
[<AllowNullLiteral>]
type ArraySeq_int(_meta: IPersistentMap, _array: int array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_int(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_int(newMeta, _array, _index)

/// Sequence over an array of uints.
[<AllowNullLiteral>]
type ArraySeq_uint(_meta: IPersistentMap, _array: uint32 array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_uint(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_uint(newMeta, _array, _index)

/// Sequence over an array of longs.
[<AllowNullLiteral>]
type ArraySeq_long(_meta: IPersistentMap, _array: int64 array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_long(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_long(newMeta, _array, _index)

/// Sequence over an array of ulongs.
[<AllowNullLiteral>]
type ArraySeq_ulong(_meta: IPersistentMap, _array: uint64 array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_ulong(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_ulong(newMeta, _array, _index)

/// Sequence over an array of float32s.
[<AllowNullLiteral>]
type ArraySeq_float(_meta: IPersistentMap, _array: float32 array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_float(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_float(newMeta, _array, _index)

/// Sequence over an array of doubles.
[<AllowNullLiteral>]
type ArraySeq_double(_meta: IPersistentMap, _array: float array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_double(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_double(newMeta, _array, _index)

/// Sequence over an array of chars.
[<AllowNullLiteral>]
type ArraySeq_char(_meta: IPersistentMap, _array: char array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_char(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_char(newMeta, _array, _index)

/// Sequence over an array of bools.
[<AllowNullLiteral>]
type ArraySeq_bool(_meta: IPersistentMap, _array: bool array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_bool(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_bool(newMeta, _array, _index)

/// Sequence over an array of decimals.
[<AllowNullLiteral>]
type ArraySeq_decimal(_meta: IPersistentMap, _array: decimal array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_decimal(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_decimal(newMeta, _array, _index)

/// Sequence over an array of objects.
[<AllowNullLiteral>]
type ArraySeq_object(_meta: IPersistentMap, _array: obj array, _index: int) =
    ...
    override this.NextOne() =
        new ArraySeq_object(_meta, _array, _index + 1)

    override this.DuplicateWithMeta(newMeta) =
        if Object.ReferenceEquals(newMeta, _meta) then
            this
        else
            new ArraySeq_object(newMeta, _array, _index)


/// Static class for creating ArraySeq instances.
