...
type APersistentSet(_impl : IPersistentMap) =
    ...
    static member setEquals (s: IPersistentSet, obj: obj): bool = 
        ...
        let equalsIPersistentSet (s1: IPersistentSet, s2: IPersistentSet) = 
            if (s1:>Counted).count() <> (s2:>Counted).count() then false
            else
        ...
        let equalsISet (s: IPersistentSet, set: ISet<Object>) = 
            if (s:>Counted).count() <> set.Count then false
            else
                (set :> IEnumerable<Object>)
                |> Seq.forall (fun o -> s.contains(o))
    ...
    interface IPersistentSet with
        member _.disjoin (key: obj): IPersistentSet = raise <| NotImplementedException("Concrete subclasses must define disjoin")
    ...
    interface ICollection<Object> with
        member _.Add(item: obj) = raise <| InvalidOperationException("Cannot modify immutable set")
        member _.Clear() = raise <| InvalidOperationException("Cannot modify immutable set")
        member this.Contains(item: obj): bool = (this :> IPersistentSet).contains(item)
        member this.CopyTo(array: obj array, arrayIndex: int) =
            ...
            for v in (this :> ICollection<Object>) do
                ...
                i <- i + 1
        member _.Remove(item: obj): bool = raise <| InvalidOperationException("Cannot modify immutable set")
    ...
    interface IEnumerable with
        member this.GetEnumerator() = (this :> IEnumerable<Object>).GetEnumerator()
    ...
    interface ITransientCollection with
        member this.conj (o: obj): ITransientCollection = 
            ...
            if not <| Object.ReferenceEquals(m,_impl) then
                _impl <- m
            this
        member this.persistent (): IPersistentCollection = raise <| NotImplementedException("Concrete subclasses must define persistent")
...
type PersistentHashSet(_meta: IPersistentMap, _impl: IPersistentMap) =
    ...
    static member create(items: ISeq) =
        ...
        let ret = loop ((PersistentHashSet.Empty :> IEditableCollection).asTransient()) items 
        ret.persistent() :?> PersistentHashSet

    static member createWithCheck ([<ParamArray>] init: obj []) = 
        let mutable ret = (PersistentHashSet.Empty :> IEditableCollection).asTransient() :?> ITransientSet
    ...
    static member createWithCheck(items: ISeq) =
        ...
        let ret = loop 0 ((PersistentHashSet.Empty :> IEditableCollection).asTransient() :?> ITransientSet) items 
        ret.persistent() :?> PersistentHashSet
...
and [<AllowNullLiteral>] TransientHashSet(_impl: ITransientMap) = 
   ...
   interface ITransientCollection with
       override this.persistent (): IPersistentCollection = PersistentHashSet(null, _impl.persistent())
