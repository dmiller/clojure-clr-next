...
type PVCons() =
    ...
    member this.FirstCons() =
        let mutable pv = clojure.lang.PersistentVector.EMPTY :> clojure.lang.IPersistentVector
    ...
    member this.NextCons() =
        let mutable pv = Clojure.Collections.PersistentVector.EMPTY :> Clojure.Collections.IPersistentVector
        for i in 0 .. this.size do
            pv <- pv.cons(i)
        pv
...
type PVNth() =
    ...
    member this.GlobalSetup() =
        ...
        let nextR : Clojure.Collections.ISeq = Clojure.Collections.LongRange.create(this.size)
