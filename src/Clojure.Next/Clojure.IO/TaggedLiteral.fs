...
/// A tagged literal, created by the reader.
[<Sealed>]
type TaggedLiteral private (_tag: Symbol, _form: obj) =
    ...
    /// Create a TaggedLiteral with the given tag and form.
    ...
    interface ILookup with
        ...
        member this.valAt(key: obj, notFound: obj) : obj =
            if TaggedLiteral.FormKeyword.Equals(key) then _form
            elif TaggedLiteral.TagKeyword.Equals(key) then _tag
            else notFound
    ...
    override this.Equals(o: obj) : bool =
        ...
        | :? TaggedLiteral as tl ->
            (if isNull _form then
                 ...
                 _form.Equals(tl.Form))
            && (if isNull _tag then isNull tl.Tag else _tag.Equals(tl.Tag))
        | _ -> false

    override _.GetHashCode() =
        Hashing.hashCombine (Hashing.hash (_tag), Hashing.hash (_form))
