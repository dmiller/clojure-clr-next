...
/// The object representing a reader conditional (when :read-cond is :preserve).
[<Sealed>]
type ReaderConditional private (_form: obj, _isSplicing: Nullable<bool>) =
    ...
    static member create(form: obj, isSplicing: bool) = ReaderConditional(form, isSplicing)
    ...
    interface ILookup with
        ...
        member _.valAt(key: obj, notFound: obj) : obj =
            if FormKeyword.Equals(key) then _form
            elif SplicingKeyword.Equals(key) then _isSplicing
            else notFound
